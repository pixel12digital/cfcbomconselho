 ...
 * Execute via SSH: php tools/run_migration_034_remote.php
 * 
...
try {
    ...
    if ($columnExists('cfcs', 'logo_path')) {
        echo "   ⏭️  Coluna 'logo_path' já existe, migration já foi executada.\n\n";
        echo "✅ VERIFICAÇÃO CONCLUÍDA - Coluna já existe no banco.\n";
        exit(0);
    } else {
        echo "   Coluna 'logo_path' não existe. Adicionando...\n";
        
        // Verificar se coluna email existe (para posicionar corretamente)
        if (!$columnExists('cfcs', 'email')) {
            echo "   ⚠️  Coluna 'email' não existe. Adicionando logo_path no final da tabela...\n";
    ...
    if ($columnExists('cfcs', 'logo_path')) {
        ...
        $stmt = $db->query("DESCRIBE cfcs logo_path");
        $columnInfo = $stmt->fetch();
        if ($columnInfo) {
            echo "   Detalhes da coluna:\n";
            echo "   - Tipo: {$columnInfo['Type']}\n";
        ...
        echo "2. Faça upload do logo do CFC\n";
        echo "3. Os ícones PWA serão gerados automaticamente\n";
    } else {
