...
// Carregar configurações - app está na raiz, não em public_html
...
try {
    ...
    try {
        ...
        if ($tableExists) {
            ...
            $auditLogs = [];
            echo "<p class='info'>ℹ️ Tabela audit_logs não existe. Pulando verificação de auditoria.</p>";
        }
    } catch (\Exception $e) {
        $auditLogs = [];
        echo "<p class='info'>ℹ️ Erro ao verificar auditoria: " . htmlspecialchars($e->getMessage()) . "</p>";
    }
