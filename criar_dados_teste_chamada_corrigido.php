<?php
/**
 * Script de Teste Manual - ETAPA 1.3: Interface de Chamada (Corrigido)
...
try {
    ...
    $aulasAnteriores = [
        ['nome' => 'Aula Anterior 1', 'data' => date('Y-m-d', strtotime('-3 days')), 'ordem' => 4],
        ['nome' => 'Aula Anterior 2', 'data' => date('Y-m-d', strtotime('-2 days')), 'ordem' => 5],
        ['nome' => 'Aula Anterior 3', 'data' => date('Y-m-d', strtotime('-1 day')), 'ordem' => 6]
    ];
    
    foreach ($aulasAnteriores as $index => $aulaAnterior) {
        $aulaAnteriorId = $db->insert('turma_aulas', [
            'turma_id' => $turmaId,
            'ordem' => $aulaAnterior['ordem'],
